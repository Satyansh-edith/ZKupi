 ...
 * Runs on PORT 4000 (separate from Next.js frontend on port 3000).
 */
...
// ─── Security Middleware ─────────────────────────────────────────────────────
app.use(helmet({
  contentSecurityPolicy: false, // relaxed for hackathon
}));

// ─── CORS — Allow Next.js frontend to call this server ───────────────────────
app.use(cors({
  origin: [
...
// ─── Logging ─────────────────────────────────────────────────────────────────
// dev = colored, combined = production-grade logs
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      transactions: "GET  /api/transactions, POST /api/transactions/submit",
      proof:        "POST /api/proof/generate, POST /api/proof/verify",
      merchant:     "POST /api/merchant/create, GET /api/merchant/:merchantId",
    },
  });
});
